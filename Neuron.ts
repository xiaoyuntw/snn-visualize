...
export class Neuron {
    ...
    threshold: number
    voltage: number
    private v_loss: number
    private v_update: number
    private t_update: number
    private alpha: number
    private beta: number // 相當於 t_loss

    private callbacks: ((t: number) => void)[]
    spiked: boolean
    ...
    registerSpike (f: (v: number) => void) {
        this.callbacks.push(f)
    }

    nexttick (dt: number) {
        if (this.hover) this.add(0.5 * this.base_threshold)

        /* 噪聲機制 */
        this.add(Math.random() * 2)
        const poissonRate = 0.005 // 1% tick 機率
        if (Math.random() < poissonRate) this.add(20) // 突發電壓
