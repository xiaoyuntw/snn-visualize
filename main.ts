...
class Pool {
    ...
    updateCanvas () {
        ...
        for (const s of this.synapses) {
            if (this.canvas == null) break
            this.synapses.forEach(s => {
                if (s.preTrace < 0.3) return
                ...
                if (s.weight < 0) 
                    ctx.strokeStyle = `rgba(255, 50, 150, ${ (-1 * s.weight * (s.preTrace ** 4))})`;
                else
                    ctx.strokeStyle = `rgba(50, 255, 150, ${(s.weight * (s.preTrace ** 4))})`;
